...
select custid, orderid, qty,
		rank()
			Over(partition by custid order by qty) as rnk,
		dense_rank() 
			over(partition by custid order by qty) as drnk
	from dbo.orders;
...
select empid, 
	count(orderid) over(partition by empid, year(orderdate)) as cnt2014
